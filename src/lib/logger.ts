...
import { asyncLocalStorage, ITraceId } from '../middleware/trace.middleware';
...
const format = winston.format.combine(
	...
	winston.format.printf((info) => {
		const store: ITraceId | undefined = asyncLocalStorage.getStore();
		const traceId = store?.traceId;
		const traceLog = traceId ? ` trace_id=${traceId} ` : ''
		return `${info.timestamp}${traceLog}${info.level}: ${info.message}`;
	})
);
